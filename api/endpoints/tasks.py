...
def _response(task: Task, dor: DORRuntimeDB) -> TaskResponse:
    ...
    return TaskResponse(
        ...
        assigned_actor=actor.to_dict() if actor else None,
...
def create_task(task: TaskCreate, dor: DORRuntimeDB = Depends(get_dor)):
    """Opret en ny Task."""
    workflow = dor.db_adapter.get_workflow(task.workflow_id) if task.workflow_id else None
    ...
    assigned_actor = (
        dor.db_adapter.get_actor(task.assigned_actor_id)
        if task.assigned_actor_id
        else None
    )
    db_task = Task(
        id=task.id,
...
def get_task(task_id: str, dor: DORRuntimeDB = Depends(get_dor)):
...
def get_tasks(
    ...
    """Hent Tasks filtreret efter workflow, actor eller status."""
    if workflow_id:
...
def assign_task(task_id: str, actor_id: str, dor: DORRuntimeDB = Depends(get_dor)):
