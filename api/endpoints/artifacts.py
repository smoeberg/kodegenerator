...
def _response(artifact: Artifact, dor: DORRuntime) -> ArtifactResponse:
    department = (
...
def create_artifact(
    ...
    import hashlib
    import json
    canonical_metadata = json.dumps(artifact.metadata or {}, sort_keys=True, separators=(",", ":")).encode("utf-8")
...
def get_artifact(artifact_id: str, dor: DORRuntime = Depends(get_dor)):
...
def get_artifacts(workflow_id: Optional[str] = None, owner_id: Optional[str] = None, dor: DORRuntime = Depends(get_dor)):
    if workflow_id:
...
def submit_artifact(artifact_id: str, actor_id: str, dor: DORRuntime = Depends(get_dor)):
    artifact = dor.db_adapter.get_artifact(artifact_id)
    if not artifact:
        raise HTTPException(status_code=404, detail="Artifact not found")
...
def approve_artifact(artifact_id: str, actor_id: str, role_id: str, dor: DORRuntime = Depends(get_dor)):
    artifact = dor.db_adapter.get_artifact(artifact_id)
    if not artifact:
        raise HTTPException(status_code=404, detail="Artifact not found")
    actor = dor.db_adapter.get_actor(actor_id)
