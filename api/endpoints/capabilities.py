...
def _response(cap: Capability) -> CapabilityResponse:
    return CapabilityResponse(id=cap.id, name=cap.name, description=cap.description, level=cap.level, certification=cap.certification, used_by=cap.used_by, created_at=cap.created_at, updated_at=cap.updated_at)


@router.post("/", response_model=CapabilityResponse, status_code=status.HTTP_201_CREATED)
def create_capability(capability: CapabilityCreate, dor: DORRuntime = Depends(get_dor)):
    db_cap = Capability(id=capability.id, name=capability.name, description=capability.description, level=capability.level, certification=capability.certification)
    return _response(dor.db_adapter.create_capability(db_cap))


@router.get("/{capability_id}", response_model=CapabilityResponse)
def get_capability(capability_id: str, dor: DORRuntime = Depends(get_dor)):
...
def get_capabilities(dor: DORRuntime = Depends(get_dor)):
