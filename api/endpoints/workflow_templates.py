...
def _response(template) -> WorkflowTemplateResponse:
    return WorkflowTemplateResponse(
        ...
        states=[state.to_dict() for state in template.states],
        transitions=[transition.to_dict() for transition in template.transitions],
        gates=[gate.to_dict() for gate in template.gates],
...
def get_workflow_templates(dor: DORRuntimeDB = Depends(get_dor)):
