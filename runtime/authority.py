...
class AuthorityRuntime:
        ...
        *,
        ...
        if role.organization_id != context.organization_id:
            ...
            decision = AuthorizationDecision(
                allowed=False,
                reason="Role definition belongs to another organization",
                reason_code="resource_not_accessible",
                actor_id=decision.actor_id,
                principal_id=decision.principal_id,
                organization_id=decision.organization_id,
                capability_id="authority.role.create",
                resource_id=role.id,
                resource_organization_id=role.organization_id,
            )
        ...
        with self.runtime.database.session() as session:
            with UnitOfWork(session) as uow:
                ...
                uow.events.append(
                    create_authority_mutation_audit_event(
                        decision,
                        command_id=command_id,
                        action="role_created",
    ...
    def assign_role(
        ...
        *,
        ...
        if not decision.allowed:
            self._deny(decision, command_id, "role_assigned")
        with self.runtime.database.session() as session:
            with UnitOfWork(session) as uow:
                ...
                uow.events.append(
                    create_authority_mutation_audit_event(
                        ...
                        target_actor_id=assignment.actor_id,
                    )
                )
        return assignment
    ...
    def _set_role_status(self, context: "OrganizationContext", role_definition_id: str, status: str, capability_id: str, action: str, command_id: str | None) -> None:
        ...
        if not decision.allowed:
            self._deny(decision, command_id, action)
        with self.runtime.database.session() as session:
            with UnitOfWork(session) as uow:
    ...
    def activate_assignment(self, context: "OrganizationContext", actor_id: str, role_definition_id: str, command_id: str | None = None) -> None:
        self._set_assignment_status(context, actor_id, role_definition_id, "active", "authority.role.activate", "role_activated", command_id)
