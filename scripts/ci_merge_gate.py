...
class MergeGateChecker:
    ...
    def check_sandbox_root_bind(self, files: List[str]) -> None:
        ...
        # Also inspect process.py or sandbox files specifically
        target_files = files + ["phase6/execution/process.py", "services/sandbox.py"]
        for f_path in set(target_files):
            ...
            try:
                ...
                # AST check for bwrap args mounting '/'
                tree = ast.parse(content)
                for node in ast.walk(tree):
                    if isinstance(node, ast.Call):
                        # check if calling something with bwrap / --ro-bind
                        pass
