...
class AuthorityRepository:
    ...
    def add_role_definition(self, role: RoleDefinition) -> None:
        self.session.add(RoleDefinitionModel(
            id=role.id,
    ...
    def get_role_definition(self, role_definition_id: str, organization_id: str) -> RoleDefinition | None:
        row = self.session.scalar(select(RoleDefinitionModel).where(
    ...
    def get_role_definition_organization(self, role_definition_id: str) -> str | None:
        """Return only the owning organization needed by the authorization boundary."""
        return self.session.scalar(select(RoleDefinitionModel.organization_id).where(
            RoleDefinitionModel.id == role_definition_id,
        ))

    def assign_role(self, assignment: RoleAssignment) -> None:
        role = self.get_role_definition(assignment.role_definition_id, assignment.organization_id)
        if role is None:
            raise ValueError("Role definition not found")
        if role.status != "active":
            raise ValueError("Cannot assign an inactive role definition")

        actor_exists = self.session.scalar(select(ActorModel.id).where(
        ...
        self.session.add(RoleAssignmentModel(
    ...
    def get_assignments(self, actor_id: str, organization_id: str) -> list[RoleAssignment]:
        rows = self.session.scalars(select(RoleAssignmentModel).where(
            RoleAssignmentModel.actor_id == actor_id,
            RoleAssignmentModel.organization_id == organization_id,
        ).order_by(RoleAssignmentModel.role_definition_id)).all()
    ...
    def get_assignment(self, actor_id: str, organization_id: str, role_definition_id: str) -> RoleAssignment | None:
        row = self.session.scalar(select(RoleAssignmentModel).where(
            RoleAssignmentModel.actor_id == actor_id,
            RoleAssignmentModel.organization_id == organization_id,
    ...
    def set_role_definition_status(self, role_definition_id: str, organization_id: str, status: str) -> None:
    ...
    def set_assignment_status(self, actor_id: str, organization_id: str, role_definition_id: str, status: str) -> None:
    ...
    def revoke_assignment(self, actor_id: str, organization_id: str, role_definition_id: str) -> None:
        self.set_assignment_status(actor_id, organization_id, role_definition_id, "revoked")

    def get_effective_capabilities(self, actor_id: str, organization_id: str) -> frozenset[str]:
        ...
        roles = {role_id: role for role_id in role_ids if (role := self.get_role_definition(role_id, organization_id)) is not None}
        return resolve_effective_capabilities(actor_id, organization_id, assignments, roles)
