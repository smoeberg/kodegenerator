...
def database_scope_validator(database) -> ScopeValidator:
    ...
    def validate(organization_id: str, project_id: str, plan_request_fingerprint: str) -> bool:
        try:
...
class ProjectScopedWorkQueueLeaseService(WorkQueueLeaseService):
    """Fence heartbeat/submission once the WorkUnit's plan is superseded."""
    ...
    def recover_next_expired(self, worker_id: str, capabilities: list[Capability]):
        if not self._scope_is_current():
            return None
        recovered = super().recover_next_expired(worker_id, capabilities)
        if recovered is None:
            return None
        try:
            binding = work_unit_scope(recovered)
        except WorkUnitContractError:
            return None
        if binding != (self.project_id, self.plan_request_fingerprint):
            return None
