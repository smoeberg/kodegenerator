...
class TestOnboardingIntentDraft:
    def test_purpose_is_closed_and_client_draft_has_no_trusted_identity_fields(self):
        ...
        with pytest.raises(OnboardingContractError, match="declared OnboardingPurpose"):
            OnboardingIntentDraft(
                source_repository=REPOSITORY,
    ...
    def test_rewrite_requires_target_stack_and_other_purposes_forbid_it(self):
        with pytest.raises(OnboardingContractError, match="requires an explicit target_stack"):
            OnboardingIntentDraft(
                source_repository=REPOSITORY,
        ...
        for purpose in (OnboardingPurpose.EXTEND, OnboardingPurpose.AUDIT_ONLY):
    ...
    def test_semantic_text_is_canonical_and_authority_resource_is_exact(
        self, source_repository, rationale, message
    ):
...
class TestRecordedOnboardingIntent:
    def test_trusted_metadata_is_attached_by_record_boundary(self):
        ...
        assert intent.declared_at == datetime(2026, 9, 5, 16, 30, tzinfo=timezone.utc)
    ...
    def test_actor_organization_and_supersession_are_bound_into_record_identity(self):
        draft = make_draft()
        baseline = OnboardingIntent.from_draft(
            draft, declared_by=ACTOR, organization_id=ORGANIZATION
        )
        other_actor = OnboardingIntent.from_draft(
            draft, declared_by="user:bob", organization_id=ORGANIZATION
        )
        other_org = OnboardingIntent.from_draft(
            draft, declared_by=ACTOR, organization_id="org:other"
        )
    ...
    def test_record_is_immutable_and_rejects_untrusted_timestamp_shape(self):
        intent = OnboardingIntent.from_draft(
            make_draft(), declared_by=ACTOR, organization_id=ORGANIZATION
        )
    ...
    def test_record_factory_rejects_non_draft_input(self):
        with pytest.raises(TypeError, match="OnboardingIntentDraft"):
            OnboardingIntent.from_draft(  # type: ignore[arg-type]
                object(), declared_by=ACTOR, organization_id=ORGANIZATION
            )


class TestOnboardingObjectives:
    def test_each_purpose_has_an_explicit_non_overlapping_objective_contract(self):
        assert objectives_for(OnboardingPurpose.EXTEND) is EXTEND_OBJECTIVES
        assert objectives_for(OnboardingPurpose.MODERNIZE_REWRITE) is MODERNIZE_REWRITE_OBJECTIVES
        assert objectives_for(OnboardingPurpose.AUDIT_ONLY) is AUDIT_ONLY_OBJECTIVES
        ...
        assert set(MODERNIZE_REWRITE_OBJECTIVES).isdisjoint(AUDIT_ONLY_OBJECTIVES)

    def test_objective_mapping_rejects_free_text_even_when_value_matches_enum(self):
        with pytest.raises(OnboardingContractError, match="declared OnboardingPurpose"):
