...
def test_registered_agent_produces_governed_patch_outcome_through_ai1_to_ai5():
    ...
    context_packet = ContextPacketEngine().build(
        ContextRequest(
            ...
            requested_keys=("src/calculator.py", "acceptance"),
            max_items=2,
            max_bytes=4_096,
        ),
        (
            ContextItem(
                source="repository",
                key="src/calculator.py",
                value="def add(left, right):\n    return left - right\n",
                provenance="git:39a44a7:src/calculator.py",
            ),
            ContextItem(
                source="requirements",
                key="acceptance",
                value="add(2, 3) returns 5",
                provenance="requirement:REQ-CALC-1",
            ),
        ),
    )
    ...
    implementation_request = ImplementationRequest(
        ...
        instruction="Correct add so it returns the sum of its arguments.",
    ...
    authority = AuthorityEngine(
        AuthorityPolicy(
            policy_id="policy.phase4b.implementation",
            version="1",
            rules=(
                AuthorityRule(
                    rule_id="allow-bounded-patch-proposal",
                    action=IMPLEMENTATION_ACTION,
                    resource_pattern="repository:smoeberg/kodegenerator",
                    effect=Decision.ALLOW,
                    agent_identity=str(agent.identity),
                    agent_role=AgentRole.EXECUTOR.value,
                    required_context=tuple(
                        sorted(implementation_request.authority_context().items())
                    ),
                ),
            ),
        )
