...
def make_outcome(status: OutcomeStatus = OutcomeStatus.FAILED, *, outcome_id: str = "outcome-1", request_id: str = "request-1") -> OutcomeRecord:
    return OutcomeRecord(outcome_id=outcome_id, execution_id="execution-1", request_id=request_id, status=status, transitions=(), provenance_id="provenance-1", produced_at="2026-08-08T00:00:00+00:00")


def make_authority(decision: Decision = Decision.ALLOW, *, request_id: str = "request-1") -> AuthorityDecision:
    return AuthorityDecision(request_id=request_id, decision=decision, agent_identity="agent-1", action="update", resource="resource-1", context_packet_id="context-1", policy_id="policy-1", policy_version="1", matched_rule_ids=("rule-1",), reason="explicit test decision", evaluated_at="2026-08-08T00:00:00+00:00", parameters=(), organization_id="org-1")


def make_plan_request(status: OutcomeStatus = OutcomeStatus.FAILED, *, attempt: int = 0, outcome_id: str = "outcome-1") -> PlanRequest:
    return PlanRequest(outcome=make_outcome(status, outcome_id=outcome_id), request_fingerprint="request-fingerprint-1", action="update", resource="resource-1", context_packet_id="context-1", parameters=(("key", "value"),), attempt=attempt)


def make_observation(status: OutcomeStatus = OutcomeStatus.FAILED, *, number: int = 1, retry_count: int = 0, authority: AuthorityDecision | None = None, processed_outcome_ids=(), outcome_id: str = "outcome-1") -> OrchestrationObservation:
    return OrchestrationObservation(iteration=IterationIdentity("run-1", number=number, retry_count=retry_count), plan_request=make_plan_request(status, attempt=retry_count, outcome_id=outcome_id), authority_decision=authority if authority is not None else make_authority(), processed_outcome_ids=processed_outcome_ids)


def test_failure_continues_only_through_ai6():
    decision = decide(make_observation(), LoopBounds(max_depth=3, max_retries=2))
    ...
    assert decision.handoff.plan_request is decision.observation.plan_request
    assert decision.outcome is decision.observation.outcome
...
@pytest.mark.parametrize(("number", "retry_count"), ((0, 0), (1, 1), (2, 0), (2, -1)))
...
def test_plan_attempt_must_match_iteration_retry_count():
    with pytest.raises(ValueError, match="plan attempt"):
        OrchestrationObservation(iteration=IterationIdentity("run-1", number=2, retry_count=1), plan_request=make_plan_request(attempt=0), authority_decision=make_authority())


@pytest.mark.parametrize(("max_depth", "max_retries", "error"), ((0, 0, ValueError), (1, -1, ValueError), (True, 0, TypeError), (1, False, TypeError)))
...
def test_loop_depth_is_bounded_before_ai6_handoff():
    decision = decide(make_observation(), LoopBounds(max_depth=1, max_retries=10))
...
def test_retries_are_bounded_before_ai6_handoff():
    decision = decide(make_observation(), LoopBounds(max_depth=10, max_retries=0))
    assert decision.directive is OrchestrationDirective.STOP
...
def test_success_is_a_terminal_stop():
    decision = decide(make_observation(OutcomeStatus.SUCCEEDED), LoopBounds(max_depth=3, max_retries=2))
    assert decision.directive is OrchestrationDirective.STOP
...
def test_denied_authority_terminates_without_planning():
    decision = decide(make_observation(OutcomeStatus.REJECTED, authority=make_authority(Decision.DENY)), LoopBounds(max_depth=3, max_retries=2))
    assert decision.directive is OrchestrationDirective.STOP
...
def test_missing_authority_fails_closed():
    observation = OrchestrationObservation(iteration=IterationIdentity("run-1"), plan_request=make_plan_request(), authority_decision=None)
...
def test_unknown_outcome_fails_closed():
    decision = decide(make_observation(OutcomeStatus.UNKNOWN), LoopBounds(max_depth=3, max_retries=2))
    assert decision.directive is OrchestrationDirective.STOP
...
def test_replayed_outcome_cannot_continue():
    decision = decide(make_observation(OutcomeStatus.REPLAYED), LoopBounds(max_depth=3, max_retries=2))
    assert decision.directive is OrchestrationDirective.STOP
    assert decision.state is OrchestrationState.DUPLICATE_OUTCOME
...
def test_authority_decision_must_be_bound_to_the_outcome_request():
    with pytest.raises(ValueError, match="request_id"):
        OrchestrationObservation(iteration=IterationIdentity("run-1"), plan_request=make_plan_request(), authority_decision=make_authority(request_id="different-request"))
...
def test_authority_subject_must_match_the_planning_subject(field):
    ...
    with pytest.raises(ValueError, match=field):
        OrchestrationObservation(iteration=IterationIdentity("run-1"), plan_request=make_plan_request(), authority_decision=authority)


def test_unrecognized_authority_value_fails_closed():
    with pytest.raises(TypeError, match="decision must be a Decision"):
        replace(make_authority(), decision="unexpected")


def test_unrecognized_outcome_value_fails_closed():
    decision = decide(make_observation("unexpected"), LoopBounds(max_depth=3, max_retries=2))
    assert decision.directive is OrchestrationDirective.STOP
    assert decision.state is OrchestrationState.OUTCOME_UNKNOWN
    assert decision.reason is DecisionReason.OUTCOME_UNKNOWN
    assert decision.handoff is None
...
def test_all_non_active_states_are_terminal():
    ...
    assert all(state.terminal for state in OrchestrationState if state is not OrchestrationState.ACTIVE)


def test_unknown_cannot_be_relabelled_as_continue():
    observation = make_observation(OutcomeStatus.UNKNOWN)
    handoff = PlannerHandoff(run_id=observation.run_id, iteration_id=observation.iteration.iteration_id, plan_request=observation.plan_request)
    with pytest.raises(ValueError, match="violates"):
        OrchestrationDecision(observation=observation, bounds=LoopBounds(max_depth=3, max_retries=2), directive=OrchestrationDirective.CONTINUE, state=OrchestrationState.ACTIVE, reason=DecisionReason.PLANNING_REQUIRED, handoff=handoff)
