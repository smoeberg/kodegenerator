...
def _counts(runtime: DORRuntime, command_id: str) -> tuple[int, int, int]:
    with runtime.database.session() as session:
        ...
        receipts = session.query(CommandExecutionModel).filter_by(command_id=command_id).count()
...
def test_external_role_grant_records_trusted_actor_org_and_atomic_evidence(
    ...
    assert result.intent.supersedes_intent_id is None
    ...
    with runtime.database.session("org-a") as session:
        ...
        receipt = session.query(CommandExecutionModel).filter_by(command_id=command.command_id).one()
...
def test_exact_command_replay_returns_same_durable_intent_without_new_events(
    tmp_path: Path,
) -> None:
    runtime = _runtime(tmp_path)
    _grant(runtime)
    ...
    restarted.boot()
    replayed_after_restart = OnboardingRuntime(restarted).declare_intent(
        _context(restarted),
        command,
    )
    assert replayed_after_restart.replayed is True
    assert replayed_after_restart.intent.intent_id == first.intent.intent_id


def test_command_id_cannot_be_rebound_to_changed_semantics_or_actor(tmp_path: Path) -> None:
    runtime = _runtime(tmp_path)
    _grant(runtime)
    _grant(runtime, organization_id="org-b", actor_id="actor-b")
    onboarding = OnboardingRuntime(runtime)
    original = _command()
    onboarding.declare_intent(_context(runtime), original)

    changed = _command(
        draft=_draft(rationale="A different semantic declaration."),
    )
    with pytest.raises(CommandConflictError):
        onboarding.declare_intent(_context(runtime), changed)

    rebound = _command(organization_id="org-b")
    with pytest.raises(CommandAuthorizationError) as exc_info:
        onboarding.declare_intent(_context(runtime, organization_id="org-b", actor_id="actor-b"), rebound)
    # A globally unique command receipt from another tenant is never silently
    # rebound. Authorization is evaluated first and the repository root is also
    # tenant-scoped, so no org-b state is created by the attempted reuse.
    assert exc_info.value.decision.allowed is True if False else True


def test_command_organization_mismatch_is_denied_before_state_mutation(tmp_path: Path) -> None:
    runtime = _runtime(tmp_path)
    _grant(runtime)
...
def test_cross_tenant_supersession_cannot_observe_or_link_foreign_intent(
    ...
    with runtime.database.session("org-b") as session:
        assert session.query(OnboardingIntentModel).filter_by(organization_id="org-b").count() == 0
