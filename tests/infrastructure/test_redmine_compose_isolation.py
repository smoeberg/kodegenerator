...
def test_redmine_secret_is_not_interpolated_into_canonical_compose() -> None:
    ...
    assert "REDMINE_API_KEY" not in api_environment
    assert "REDMINE_API_KEY_FILE" not in api_environment
    ...
    for service_name in ("dashboard", "worker", "migrate"):
        ...
        assert "REDMINE_API_KEY_FILE" not in environment
