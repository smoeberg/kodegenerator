...
def contract(*, mandatory=True):
    return Contract(
        contract_id="P5-00-TEST",
        contract_version="1.0.0",
        product_type="test-product",
        product_location="phase5/p5-00-ai-work-product-contract",
        intent="prove governed work-product acceptance",
        inputs=("test-input",),
        required_artifacts=(A("impl", AT.FILE, "src/impl.py"),),
        outputs=("implemented artifact",),
        ...
        regression_requirements=("existing suite remains green",),
        required_capabilities=("repository-read",),
        authority_boundaries=("agent cannot decide PASS",),
        forbidden_actions=("mutate contract after dispatch",),
...
def verify(c, s, *, passed=True):
    return VE().verify(
        c, s, (fact(),), {"impl": "artifact-hash"},
        decision_id="DEC-1", predicates={"AC-1": lambda facts: passed},
...
def test_completion_claim_is_not_authoritative():
    c = contract()
    d = verify(c, submission(c), passed=False)
...
def test_contract_fingerprint_mismatch_fails_closed():
    c = contract()
    s = submission(c)
    object.__setattr__(s, "contract_fingerprint", "tampered")
    try:
        verify(c, s)
        assert False
    except VErr:
...
def test_required_artifact_missing_fails():
    c = contract()
    s = WP(
        submission_id="SUB-2", contract_fingerprint=c.contract_fingerprint, agent_id="agent-1",
        repository_state=RS("repo", "rev", "tree", True), artifacts=(), candidate_evidence=(),
...
def test_artifact_fingerprint_mismatch_fails():
    c = contract()
    d = verify(c, submission(c, artifact_fingerprint="wrong"))
    assert d.passed is False
...
def test_artifact_metadata_mismatch_fails():
    c = contract()
    s = WP(
        submission_id="SUB-3", contract_fingerprint=c.contract_fingerprint, agent_id="agent-1",
        repository_state=RS("repo", "rev", "tree", True),
        artifacts=(SA("impl", AT.FILE, "wrong.py", "artifact-hash"),), candidate_evidence=(),
        submitted_at=datetime.now(timezone.utc),
    )
    d = verify(c, s)
    assert d.passed is False
...
def test_candidate_evidence_cannot_become_authoritative():
    c = contract()
    d = verify(c, submission(c, candidate=True))
    assert d.passed is True
...
def test_missing_predicate_fails_closed():
    c = contract()
    d = VE().verify(c, submission(c), (fact(),), {"impl": "artifact-hash"}, decision_id="DEC-3")
    assert d.passed is False
...
def test_lifecycle_is_append_only_and_p3_20_gated():
    e = module.LifecycleEvent
    append = module.append_event
    state = module.derive_delivery_state
    events = ()
    for i, event_type in enumerate((module.DeliveryState.DISPATCHED, module.DeliveryState.IN_PROGRESS, module.DeliveryState.SUBMITTED)):
        events = append(events, e(str(i), "SUB-1", event_type, "agent-1", datetime.now(timezone.utc)))
    assert state(events) is module.DeliveryState.SUBMITTED
    try:
        append(events, e("4", "SUB-1", module.DeliveryState.VERIFYING, "agent-1", datetime.now(timezone.utc)))
        assert False
    except PermissionError:
        pass
    events = append(events, e("4", "SUB-1", module.DeliveryState.VERIFYING, "p3-20", datetime.now(timezone.utc)))
    events = append(events, e("5", "SUB-1", module.DeliveryState.PASSED, "p3-20", datetime.now(timezone.utc)))
    assert state(events) is module.DeliveryState.PASSED
...
def test_failed_submission_is_terminal():
    ...
    try:
        module.append_event(events, e("6", "SUB-1", module.DeliveryState.IN_PROGRESS, "agent-1", datetime.now(timezone.utc)))
        assert False
    except ValueError:
        pass
