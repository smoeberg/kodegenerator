...
def _runtime(tmp_path: Path) -> DORRuntime:
    runtime = DORRuntime(f"sqlite:///{tmp_path / 'p3-10.db'}")
    runtime.boot()
    return runtime
...
def _grant_transition(runtime: DORRuntime, organization_id: str, actor_id: str) -> None:
    role = RoleDefinition(
        id=f"workflow.operator.{organization_id}.{actor_id}",
...
def test_identical_decisions_have_identical_fingerprint() -> None:
    kwargs = dict(
        ...
        resource_id="workflow-a",
        resource_organization_id="org-a",
    )
    first = AuthorizationDecision(**kwargs)
    second = AuthorizationDecision(**kwargs)
...
def test_audit_event_preserves_decision_integrity(tmp_path: Path) -> None:
    ...
    assert grant.metadata["resource_id"] == workflow.id
    assert grant.metadata["resource_organization_id"] == context.organization_id
    assert len(grant.metadata["decision_fingerprint"]) == 64
...
def test_audit_builder_rejects_outcome_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="Audit outcome must match"):
        create_authorization_audit_event(
            decision,
            command_id="cmd-a",
            command_type="AdvanceWorkflowCommand",
            allowed=True,
        )
