"""P3-13 API boundary tests for canonical workflow command authorization."""
...
from api.endpoints.workflows import transition_workflow
...
def _runtime(tmp_path: Path) -> DORRuntime:
    runtime = DORRuntime(f"sqlite:///{tmp_path / 'p3-13-api.db'}")
    runtime.boot()
    return runtime
...
def _context(runtime: DORRuntime, organization_id: str, actor_id: str):
    ...
    runtime.register_actor(
        Actor(id=actor_id, type=ActorType.HUMAN, identity=actor_id), organization_id
    )
...
def test_authorized_api_command_uses_path_workflow_id(tmp_path: Path) -> None:
    ...
    _grant_transition(runtime, "org-a", "actor-a")
    ...
    response = transition_workflow(
        ...
        WorkflowTransitionRequest(
            ...
            new_state=WorkflowStateEnum.ANALYSIS,
        ),
        _user("actor-a"),
        runtime,
    )
    ...
    assert response.current_state == WorkflowStateEnum.ANALYSIS
...
def test_unauthorized_api_command_is_denied_and_audited(tmp_path: Path) -> None:
    ...
    assert exc.value.detail["reason_code"] == "capability_not_granted"
...
def test_api_cannot_substitute_workflow_id_from_request_body(tmp_path: Path) -> None:
    ...
    context = _context(runtime, "org-a", "actor-a")
    _grant_transition(runtime, "org-a", "actor-a")
    workflow_a = runtime.create_workflow(context, "workflow-a")
    workflow_b = runtime.create_workflow(context, "workflow-b")

    response = transition_workflow(
        workflow_a.id,
        WorkflowTransitionRequest(
            organization_id="org-a",
            command_id="api-path-authority",
            new_state=WorkflowStateEnum.ANALYSIS,
        ),
        _user("actor-a"),
        runtime,
    ...
    assert response.id == workflow_a.id
    assert runtime.get_workflow(context, workflow_a.id).current_state.name == WorkflowState.ANALYSIS
    assert runtime.get_workflow(context, workflow_b.id).current_state.name == WorkflowState.NEW
