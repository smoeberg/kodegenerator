...
def test_event_durability(tmp_path: Path):
    ...
    assert [event.event_type.name for event in before_restart] == ["WORKFLOW_CREATED", "WORKFLOW_STATE_CHANGED"]
...
def test_principal_cannot_bind_to_another_actor(tmp_path: Path):
    ...
    with pytest.raises(ContextError):
        runtime.establish_context(Principal(id="actor-a", type="user", metadata={"actor_id": "actor-b"}), "org-a", "actor-a")
