...
def _load_signing_key() -> bytes:
    """Load an operator-owned key or use an ephemeral fail-closed local key."""
    ...
    try:
        ...
        raise RuntimeError(
            "DOR_AUTHORITY_SIGNING_KEY must be URL-safe base64"
        ) from exc
    if len(key) < 32:
        raise RuntimeError(
...
def _attach_decision_provenance(decision: AuthorityDecision) -> None:
    """Attach issuer-authenticated provenance to a decision created by AI-3."""
...
class VerifiedAuthorityGrant:
    """Short-lived, issuer-authenticated authority credential for AI-4.
    ...
    def verify(self, *, at: datetime | None = None) -> bool:
        """Verify issuer, signature, and the inclusive/exclusive grant lifetime."""
