...
def _validate_repository_path(path: str) -> str:
    ...
    if path != path.strip() or "\\" in path:
        raise ImplementationContractError("repository paths must be canonical POSIX paths")
    ...
    if any(part in {"", ".", ".."} for part in candidate.parts):
        raise ImplementationContractError("repository paths cannot contain traversal segments")
...
class ImplementationRequest:
    """Bounded, tenant-bound request for a patch proposal."""
    organization_id: str
    agent_identity: str
    agent_role: str
    resource: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.context_packet.agent_identity != self.agent_identity:
            raise ImplementationContractError("context packet agent identity does not match the implementation request")
        if self.context_packet.purpose != IMPLEMENTATION_ACTION:
            raise ImplementationContractError(f"context packet purpose must be {IMPLEMENTATION_ACTION!r}")
    ...
    def scope_fingerprint(self) -> str:
        return _canonical_digest({"organization_id": self.organization_id, "paths": list(self.allowed_paths)})
...
class PatchProposal:
    ...
    def __post_init__(self) -> None:
        ...
        outside_scope = tuple(path for path in touched_paths if path not in self.request.allowed_paths)
        if outside_scope:
            raise InvalidPatchError("patch touches paths outside the approved scope: " + ", ".join(outside_scope))
...
def _inspect_unified_diff(unified_diff: str) -> tuple[tuple[str, ...], int]:
    ...
    in_hunk = False
    expected_old_lines = expected_new_lines = seen_old_lines = seen_new_lines = 0
    ...
    def finish_section() -> None:
        ...
        if old_header is None or new_header is None or not saw_hunk:
            raise InvalidPatchError("each diff section requires file headers and a hunk")
        expected_old, expected_new = f"a/{current_path}", f"b/{current_path}"
        if old_header not in {expected_old, "/dev/null"} or new_header not in {expected_new, "/dev/null"}:
            raise InvalidPatchError("file headers do not match the diff path")
        if old_header == "/dev/null" and new_header == "/dev/null":
            raise InvalidPatchError("a diff cannot create and delete the same null file")
    ...
    for line in lines:
        ...
        if match:
            ...
            if old_path != new_path:
                raise InvalidPatchError("renames are outside the Phase 4B-1 patch contract")
            try:
            ...
            if current_path in touched:
                raise InvalidPatchError("a patch cannot contain duplicate file sections")
        ...
        if line.startswith("@@"):
            ...
            seen_old_lines = seen_new_lines = 0
            saw_hunk = in_hunk = True
            continue
        if in_hunk:
            if line.startswith("+"):
                changed_lines += 1; section_changed_lines += 1; seen_new_lines += 1; continue
            if line.startswith("-"):
                changed_lines += 1; section_changed_lines += 1; seen_old_lines += 1; continue
            if line.startswith(" "):
                seen_old_lines += 1; seen_new_lines += 1; continue
