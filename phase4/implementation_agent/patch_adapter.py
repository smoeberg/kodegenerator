...
_EXCLUDED_FILE_NAMES = frozenset({
    ".env", ".env.local", ".env.production", ".coverage", "dor_runtime.db",
})
...
class WorkspacePatchExecutor:
    ...
    def execute(self, request: PatchExecutionRequest) -> PatchExecutionRecord:
        ...
        with self._lock:
            # Check the authority-bound live baseline before doing any patch or tool work.
            ...
            try:
                ...
                for tool in request.tools:
                    ...
                    if not item.passed:
                        detail = item.stderr.content or item.stdout.content
                        suffix = f": {detail}" if detail else ""
                        return self._failed(
                            request,
                            artifact,
                            tuple(evidence),
                            f"trusted {tool.kind.value} tool {tool.tool_id!r} finished with status {item.status.value}{suffix}",
                        )
    ...
    def _copy_workspace(self, destination: Path) -> None:
        ...
        for current, directory_names, file_names in os.walk(
            ...
            for name in sorted(directory_names):
                ...
                if stat.S_ISLNK(info.st_mode):
                    raise PatchWorkspaceError(
                        f"workspace contains symlink directory: {(relative / name).as_posix()}"
                    )
                if not stat.S_ISDIR(info.st_mode):
                    raise PatchWorkspaceError(
                        f"unsafe workspace directory: {(relative / name).as_posix()}"
                    )
            ...
            for name in sorted(file_names):
                ...
                if stat.S_ISLNK(info.st_mode):
                    raise PatchWorkspaceError(
                        f"workspace contains symlink file: {relative_file.as_posix()}"
                    )
                if not stat.S_ISREG(info.st_mode):
                    raise PatchWorkspaceError(
                        f"unsafe workspace file: {relative_file.as_posix()}"
                    )
    ...
    def _artifact_from(self, root: Path, request: PatchExecutionRequest) -> PatchArtifact:
        states = tuple(
            self._snapshot(root, path).state
            for path in request.proposal.touched_paths
        )
    ...
    def _snapshot(self, root: Path, path: str) -> _FileSnapshot:
        ...
        if stat.S_ISLNK(info.st_mode):
            raise PatchWorkspaceError(f"workspace path is a symlink: {path}")
        if not stat.S_ISREG(info.st_mode):
            raise PatchWorkspaceError(f"workspace path is not a regular file: {path}")
        if info.st_nlink > 1:
            raise PatchWorkspaceError(f"workspace path uses hard links: {path}")
...
def canonical_python_tools(
    *, timeout_seconds: int = 300, max_output_bytes: int = 256 * 1024
...
def _tool_evidence(
    tool: TrustedToolSpec, artifact_id: str, result: RawToolResult
) -> ToolEvidence:
    return ToolEvidence(
