...
class ImplementationAgentRuntimeError(RuntimeError):
    """Base error for the operational implementation-agent boundary."""


class ImplementationAgentAuthorityError(ImplementationAgentRuntimeError):
    """AI-3 denied the exact bounded implementation request."""
...
class ImplementationCommandConflictError(ImplementationAgentRuntimeError):
    """One command identity was reused for a different immutable request."""


class ImplementationContextLimitError(ImplementationAgentRuntimeError):
    """The complete eligible context does not fit within runtime bounds."""


@dataclass(frozen=True)
class ImplementationAgentRun:
    """Patch proposal plus its exact AI-1 through AI-5 provenance."""
...
class ImplementationAgentRuntime:
    """Authorize and execute bounded patch proposals through one runtime."""
    ...
    def __init__(self, *, provider: ImplementationProvider, allowed_resources: Iterable[str], max_files: int = 8, max_changed_lines: int = 1_000, max_context_items: int = 200, max_context_bytes: int = 512 * 1024) -> None:
        ...
        for resource in resources:
            if not isinstance(resource, str) or not resource.strip() or resource != resource.strip() or any(character in resource for character in "*?["):
        ...
        self._allowed_resources = resources
        ...
        self._context = ContextPacketEngine()
        self._registry = AgentRegistry()
        self._agent = self._register_agent(provider_id)
        self._authority = AuthorityEngine(self._policy_for(resources))
        self._adapter = ImplementationExecutionAdapter(adapter_id=f"adapter.implementation.runtime:{provider_id}", provider=provider)
    ...
    def run(self, *, resource: str, instruction: str, allowed_paths: tuple[str, ...], context_items: Iterable[ContextItem], budget: ChangeBudget, idempotency_key: str) -> ImplementationAgentRun:
        if not isinstance(resource, str) or not resource.strip() or resource != resource.strip() or any(character in resource for character in "*?["):
        ...
        if budget.max_files > self._max_files:
            raise ValueError("request exceeds the runtime file budget")
        if budget.max_changed_lines > self._max_changed_lines:
            raise ValueError("request exceeds the runtime changed-line budget")
        if not isinstance(idempotency_key, str) or not idempotency_key.strip() or idempotency_key != idempotency_key.strip():
            raise ValueError("idempotency_key must be a canonical non-empty string")
        items = tuple(context_items)
        if not items:
            raise ValueError("context_items must not be empty")
        if any(not isinstance(item, ContextItem) for item in items):
            raise TypeError("context_items must contain ContextItem values")
        if any(item.sensitivity == "sensitive" for item in items):
            raise ValueError("sensitive context requires an explicit future operator policy")
        ...
        if packet.truncated:
            raise ImplementationContextLimitError("eligible implementation context exceeds runtime bounds")
        request = ImplementationRequest(agent_identity=str(self._agent.identity), agent_role=self._agent.role.value, resource=resource, context_packet=packet, instruction=instruction, allowed_paths=allowed_paths, budget=budget)
        ...
        # AI-3 produces the policy decision; only its verified execution grant
        # is allowed to cross the AI-3 -> AI-4 execution boundary.
        ...
        if execution.status not in {ExecutionStatus.SUCCEEDED, ExecutionStatus.REPLAYED} or outcome.status not in {OutcomeStatus.SUCCEEDED, OutcomeStatus.REPLAYED}:
            raise ImplementationAgentExecutionError(execution, outcome)
        ...
        if proposal.request_fingerprint != request.request_fingerprint:
            raise ImplementationAgentExecutionError(execution, outcome)
        return ImplementationAgentRun(agent_identity=str(self._agent.identity), context_packet=packet, request=request, authority=authority, execution=execution, outcome=outcome, proposal=proposal)
    ...
    def _policy_for(self, resources: tuple[str, ...]) -> AuthorityPolicy:
        rules = tuple(AuthorityRule(rule_id=("allow-bounded-implementation-proposal-" + hashlib.sha256(resource.encode("utf-8")).hexdigest()[:16]), action=IMPLEMENTATION_ACTION, resource_pattern=resource, effect=Decision.ALLOW, agent_identity=str(self._agent.identity), agent_role=self._agent.role.value) for resource in resources)
        return AuthorityPolicy(policy_id="policy.implementation-agent.runtime", version="1", rules=rules)
    ...
    def _bind_command(self, idempotency_key: str, request_fingerprint: str) -> None:
        with self._lock:
            ...
            if existing is not None and existing != request_fingerprint:
                raise ImplementationCommandConflictError("idempotency key is already bound to another request")
            self._commands[idempotency_key] = request_fingerprint

    def _register_request_once(self, request: ImplementationRequest) -> None:
        with self._lock:
            if request.request_fingerprint in self._registered_requests:
                return
            self._adapter.register_request(request)
            self._registered_requests.add(request.request_fingerprint)
