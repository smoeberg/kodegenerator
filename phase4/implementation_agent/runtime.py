...
class ImplementationAgentRuntime:
    ...
    Allowed repository resources and hard budgets are supplied by the
    application operator when the runtime is constructed. They cannot be
    widened by an API caller, implementation request, or provider response.
    """
        ...
        *,
        ...
        for resource in resources:
            if (
                ...
                raise ValueError(
                    "allowed resources must be canonical exact strings without globs"
                )
        for name, value in (
            ("max_files", max_files),
        ...
        self._adapter = ImplementationExecutionAdapter(
    ...
    def run(
        ...
        *,
        ...
        if any(item.sensitivity == "sensitive" for item in items):
            raise ValueError(
                "sensitive context requires an explicit future operator policy"
            )
        packet = self._context.build(
            ContextRequest(
                agent_identity=str(self._agent.identity),
                purpose=IMPLEMENTATION_ACTION,
                requested_keys=tuple(sorted({item.key for item in items})),
                max_items=self._max_context_items,
                max_bytes=self._max_context_bytes,
            ),
            items,
            actor="implementation-agent-runtime",
        )
        if packet.truncated:
            raise ImplementationContextLimitError(
                "eligible implementation context exceeds runtime bounds"
            )
    ...
    def _policy_for(self, resources: tuple[str, ...]) -> AuthorityPolicy:
        rules = tuple(
            AuthorityRule(
                rule_id=(
                    "allow-bounded-implementation-proposal-"
        ...
        return AuthorityPolicy(
            policy_id="policy.implementation-agent.runtime",
            version="1",
            rules=rules,
        )
    ...
    def _bind_command(self, idempotency_key: str, request_fingerprint: str) -> None:
        with self._lock:
            ...
            if existing is not None and existing != request_fingerprint:
                raise ImplementationCommandConflictError(
