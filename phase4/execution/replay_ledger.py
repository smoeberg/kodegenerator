"""P4-01 execution replay ledger with leases and fencing tokens."""
    ...
    def try_claim(self, execution_id: str, *, grant_id: str | None = None, request_id: str | None = None, now: datetime | None = None) -> ClaimOutcome:
        ...
        with self._lock:
            ...
            if current is not None and current.status is LedgerStatus.SUCCEEDED:
                return ClaimOutcome(ClaimOutcomeKind.ALREADY_SUCCEEDED, current)
            if current is not None and current.status is LedgerStatus.PENDING and not _lease_expired(current.lease_expires_at, instant):
                return ClaimOutcome(ClaimOutcomeKind.IN_FLIGHT, current)
            token = _new_fencing_token()
            record = LedgerRecord(execution_id, LedgerStatus.PENDING, None, grant_id, request_id, lease_until, token)
            self._records[execution_id] = record
            return ClaimOutcome(ClaimOutcomeKind.ACQUIRED, record)

    def _require_pending_token(self, execution_id: str, fencing_token: str, op: str) -> LedgerRecord:
        ...
        if current.fencing_token != fencing_token:
            raise StaleClaimTokenError(f"{op} fencing token mismatch for {execution_id!r}")
        return current

    def complete_succeeded(self, execution_id: str, result: ExecutionResult, *, fencing_token: str) -> None:
        with self._lock:
            current = self._require_pending_token(execution_id, fencing_token, "complete_succeeded")
            if result.status is not ExecutionStatus.SUCCEEDED:
                raise ValueError("complete_succeeded requires SUCCEEDED result")
            self._records[execution_id] = replace(current, status=LedgerStatus.SUCCEEDED, result=result, lease_expires_at=None, fencing_token=None)

    def complete_failed(self, execution_id: str, result: ExecutionResult, *, fencing_token: str) -> None:
        with self._lock:
            current = self._require_pending_token(execution_id, fencing_token, "complete_failed")
            if result.status is not ExecutionStatus.FAILED:
                raise ValueError("complete_failed requires FAILED result")
            self._records[execution_id] = replace(current, status=LedgerStatus.FAILED, result=result, lease_expires_at=None, fencing_token=None)
    ...
    def abandon(self, execution_id: str, *, fencing_token: str) -> None:
        with self._lock:
            ...
            if not fencing_token or current.fencing_token != fencing_token:
                raise StaleClaimTokenError(f"abandon fencing token mismatch for {execution_id!r}")
