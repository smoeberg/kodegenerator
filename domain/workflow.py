...
class Workflow:
    ...
    def can_transition(self, new_state: WorkflowState, actor: "Actor", artifact: Optional["Artifact"] = None) -> bool:
        if not self.current_state:
            initial_states = [s for s in self.states if s.is_initial]
            return new_state in [s.name for s in initial_states]
    ...
    def transition_to(self, new_state: WorkflowState, actor: "Actor", artifact: Optional["Artifact"] = None, evidence: Optional[Dict[str, Any]] = None) -> List["Event"]:
        if not self.can_transition(new_state, actor, artifact):
            raise InvalidTransitionError(f"Cannot transition from {self.current_state.name if self.current_state else 'None'} to {new_state}")
        ...
        from domain.event import WorkflowStateChangedEvent
    ...
    def apply_event(self, event: "Event") -> None:
        from domain.event import EventType, WorkflowStateChangedEvent
        
        if isinstance(event, WorkflowStateChangedEvent):
            new_state_obj = next((s for s in self.states if s.name == event.new_state), None)
            if new_state_obj:
                self.current_state = new_state_obj
            
            if event.new_state in [WorkflowState.APPROVED, WorkflowState.RELEASED, WorkflowState.ARCHIVED]:
                self.status = WorkflowStatus.COMPLETED
            elif event.new_state == WorkflowState.REJECTED:
