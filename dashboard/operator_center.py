...
This is a presentation shell over the existing authenticated FastAPI Control
Plane. It deliberately contains no workflow authority or persistence logic.
"""
...
from dashboard.context_navigation import render_context_navigation, render_sidebar_organization_switcher, sync_organization_context
...
def login() -> None:
    st.markdown('<div class="topline"><div class="eyebrow">DIGITAL ORGANIZATION RUNTIME</div><h1>DOR / Guide</h1><div class="muted">Operatørcenter for det API-styrede Control Plane.</div></div>', unsafe_allow_html=True)
    with st.form("login"):
...
def realtime_pump(workflow_id: str) -> None:
    ...
    if manager.status == "unauthorized":
        stop_realtime(); clear_auth(); st.warning("API-session udløbet. Log ind igen."); st.rerun()
    if events:
...
def metric(title: str, value: object, foot: str) -> None:
...
def _stage(state: str) -> int:
    # Presentation mapping only; backend state remains authoritative.
    value = state.lower()
...
def overview(client: DORAPIClient, projects: list[dict[str, Any]], executions: list[dict[str, Any]]) -> None:
    ...
    st.markdown(f'<div class="topline"><div class="eyebrow">OPERATØRCENTER / OVERBLIK</div><h1>Godmorgen, {st.session_state.get("username") or "operatør"}</h1><div class="muted">Her er det, der kræver, at du bliver først.</div></div>', unsafe_allow_html=True)
    c = st.columns(4)
    with c[0]: metric("Aktive executions", len(active), f"{len(projects)} projekter")
    with c[1]: metric("Åbne beslutninger", len(attention), "Backend-ejede blocking states")
    with c[2]: metric("Gennemløb", len(executions), f"{len(executions)-len(active)} afsluttede")
    with c[3]: metric("Blocking", len(blocking), "Gates der holder progression tilbage")

    left, right = st.columns([2.15, 1])
    with left:
        if selected and execution:
            st.markdown('<div class="project-card">', unsafe_allow_html=True)
            if execution.get("action_required") in {"human_decision", "rejected"}:
            ...
            action = execution.get("action_required") or "none"
            action_text = {
                "human_decision": "Læs beslutningssporet og verificér, at anbefalingen matcher kravene.",
                "rejected": "Gennemgå afvisningen; brug kun backend-tilladt retry eller rework.",
                "rework_active": "Governed rework kører. Afvent backend-resultatet.",
                "work_in_progress": "Arbejde kører. Følg execution og evidens.",
                "terminal": "Execution er afsluttet. Gennemgå leveringsbeviset.",
            }.get(str(action), "Ingen human handling kræves lige nu.")
            st.markdown(f'<div class="attention"><b>Operatørens næste</b><br>{action_text}</div>', unsafe_allow_html=True)
            a, b = st.columns(2)
            with a:
                if st.button("Åbn projekt →", type="primary", use_container_width=True):
                    st.session_state["active_nav"] = "Projekter"; st.rerun()
            with b:
                if st.button("Åbn execution", use_container_width=True):
                    st.session_state["selected_workflow_id"] = execution.get("workflow_id"); st.session_state["active_nav"] = "Execution"; st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
        else:
            st.info("Ingen aktiv projekt/execution er rapporteret af backend.")
    with right:
        st.markdown('<div class="project-card"><div class="eyebrow">SYSTEMHELDBRED</div><h3>Rolig drift</h3>', unsafe_allow_html=True)
        try:
            ready = client.readiness(); state = ready.get("status", "unknown") if isinstance(ready, Mapping) else "unknown"
            st.metric("API / database", status_badge(state))
        except DORAPIError as exc:
            st.error(f"Readiness ({exc.status_code}): {exc}")
        try:
            ...
            st.caption(f"Operations health ikke tilgængelig ({exc.status_code}).")
...
def project_view(client: DORAPIClient, projects: list[dict[str, Any]]) -> None:
    st.markdown('<div class="topline"><div class="eyebrow">OPERATØRCENTER / PROJEKTER</div><h1>Projekter</h1><div class="muted">Intent og launch håndteres af Control Plane API.</div></div>', unsafe_allow_html=True)
    if projects:
        for project in projects:
            with st.container(border=True):
                st.markdown(f"### {project.get('name', project.get('project_id'))}")
                st.caption(f"`{project.get('project_id')}` · {status_badge(project.get('status'))} · fingerprint `{project.get('project_fingerprint','')[:16]}…`")
                st.write(project.get("description") or project.get("intent", {}).get("goal", "Ingen beskrivelse"))
                a, b = st.columns([1, 1])
                with a:
                    if st.button("Åbn", key=f"project-open-{project.get('project_id')}", type="primary"):
                        st.session_state["selected_project_id"] = project.get("project_id"); st.rerun()
                with b:
    ...
    else: st.info("Ingen projekter i den aktive organisation.")
    with st.expander("Opret projekt"):
        with st.form("operator-create-project"):
            name = st.text_input("Navn"); goal = st.text_area("Mål"); description = st.text_area("Beskrivelse")
...
def decision_view(client: DORAPIClient, executions: list[dict[str, Any]]) -> None:
    st.markdown('<div class="topline"><div class="eyebrow">OPERATØRCENTER / BESLUTNINGER</div><h1>Beslutninger</h1><div class="muted">Human-in-the-loop — kun backend-tilladte actions.</div></div>', unsafe_allow_html=True)
...
def execution_view(client: DORAPIClient) -> None:
    st.markdown('<div class="topline"><div class="eyebrow">OPERATØRCENTER / EXECUTION</div><h1>Execution & Decision Cockpit</h1><div class="muted">Execution API er eneste authority for state transitions.</div></div>', unsafe_allow_html=True)
    ...
    st.session_state["selected_workflow_id"] = workflow_id.strip(); workflow_id = workflow_id.strip()
    manager = ensure_realtime(workflow_id); realtime_pump(workflow_id)
    ...
    try:
        ...
        with st.expander("Tasks"):
            st.dataframe(summary.get("tasks", []), use_container_width=True, hide_index=True)
    ...
    with st.expander("Evidence Trace"):
        try:
            gates = client.get(f"/api/v1/execution/{workflow_id}/gates"); proposals = client.get(f"/api/v1/execution/{workflow_id}/proposals"); payload = client.get(f"/api/v1/execution/{workflow_id}")
...
def evidence_view(client: DORAPIClient) -> None:
    st.markdown('<div class="topline"><div class="eyebrow">OPERATØRCENTER / EVIDENS</div><h1>Evidens</h1><div class="muted">Read-only, tenant-scoped evidence fra API.</div></div>', unsafe_allow_html=True)
    ...
    if st.button("Hent evidens", type="primary"):
        ...
        try:
            result = client.get(f"/api/v1/bot-evidence/{mapping[label]}/{identity.strip()}", params={"organization_id": st.session_state.get("organization_id")}); st.success("Evidens hentet."); st.json(result)
        except DORAPIError as exc: st.error(f"Evidence API ({exc.status_code}): {exc}")


def main() -> None:
    inject_theme()
    if not authenticated(): stop_realtime(); login(); return
    client = api()
    try:
    ...
    if st.sidebar.button("Log ud"): stop_realtime(); clear_auth(); st.rerun()

    if nav == "Overblik": stop_realtime(); overview(client, projects, executions)
    elif nav == "Beslutninger": stop_realtime(); decision_view(client, executions)
    elif nav == "Projekter": stop_realtime(); project_view(client, projects)
    elif nav == "Execution": execution_view(client)
    elif nav == "Evidens": stop_realtime(); evidence_view(client)
    elif nav == "Governance": stop_realtime(); render_multi_bot_control_plane(client, org or "") if org else st.warning("Vælg en organisation.")
    else: stop_realtime(); render_redmine_integration(client)
