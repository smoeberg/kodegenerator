...
if menu == "🎛️ Decision Cockpit (HITL)":
...
# --- Sektion: Overblik & Systemtilstand ---
    ...
    if not conn:
        st.warning(f"Databasefilen '{DB_PATH}' blev ikke fundet. Kør systemet først for at initialisere data.")
    else:
...
# --- Sektion: Opret Ny Agent (Wizard) ---
    ...
    st.info("Brug denne guide til at registrere og konfigurere en ny AI-agent med passende rolle og sikkerhedsbegrænsninger.")
    ...
    with st.form("create_agent_form"):
        ...
        with col2:
            model_name = st.selectbox("Underliggende AI Model", ["claude-3-7-sonnet", "claude-3-5-sonnet", "gpt-4o", "mistral-large", "custom"])
        ...
        selected_caps = st.multiselect("Tilknyttede Capabilities", STANDARD_CAPABILITIES, default=default_caps)
...
# --- Sektion: Opgaver ---
    ...
    if conn:
        try:
            tasks_df = pd.read_sql("SELECT * FROM tasks ORDER BY created_at DESC LIMIT 100", conn)
...
# --- Sektion: Audit Log & Hændelser ---
    ...
    if conn:
        try:
            events_df = pd.read_sql("SELECT * FROM event_log ORDER BY timestamp DESC LIMIT 200", conn)
